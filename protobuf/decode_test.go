...
func TestDecode_GetContractAddressWithTo(t *testing.T) {
	...
	address, err := msg.getContractAddress()
	assert.NoError(t, err)
...
func TestDecode_createAddress2(t *testing.T) {
	...
	address, err := createAddress(addr, nonce)
	assert.NoError(t, err)
	assert.Equal(t, types.Address{0x9a, 0x53, 0x6c, 0xde, 0xb1, 0x3e, 0xc1, 0xa0, 0xc9, 0x3e, 0xcc, 0x66, 0xc9, 0xe2, 0x53, 0x47, 0x9d, 0x85, 0xde, 0x29}, address)

	addr = types.Address{1, 2, 3, 4}
	nonce = uint64(0)
	address, err = createAddress(addr, nonce)
	assert.NoError(t, err)
	assert.Equal(t, types.Address{0xc7, 0x68, 0x3a, 0x8a, 0x54, 0xb2, 0x1d, 0x7e, 0x5b, 0x96, 0x24, 0xa8, 0xaf, 0xe6, 0x1c, 0x2d, 0x9b, 0x25, 0x96, 0x7c}, address)

	addr = types.Address{1, 2, 3, 4}
	nonce = uint64(1)
	address, err = createAddress(addr, nonce)
	assert.NoError(t, err)
	assert.Equal(t, types.Address{0x29, 0xdb, 0x64, 0xcb, 0xeb, 0x87, 0xc2, 0xde, 0x17, 0x4, 0x5b, 0x6f, 0x67, 0xe4, 0x1e, 0x88, 0xab, 0xba, 0x6b, 0x1}, address)

	addr = types.Address{1, 2, 3, 4}
	nonce = uint64(65535)
	address, err = createAddress(addr, nonce)
	assert.NoError(t, err)
	assert.Equal(t, types.Address{0x3d, 0x92, 0xa2, 0xfc, 0xc7, 0x37, 0xac, 0x3c, 0xce, 0xd4, 0x1, 0x1b, 0x81, 0x38, 0xf, 0xcc, 0xc5, 0x1e, 0x21, 0x7a}, address)

	addr = types.Address{1, 2, 3, 4}
	nonce = uint64(32767)
	address, err = createAddress(addr, nonce)
	assert.NoError(t, err)
	assert.Equal(t, types.Address{0x5e, 0xb5, 0xeb, 0x5e, 0xf, 0x41, 0xd8, 0xdd, 0x9a, 0x4f, 0x24, 0xf3, 0x5d, 0x36, 0xed, 0x6b, 0x8f, 0x5d, 0xd1, 0xa9}, address)
}
