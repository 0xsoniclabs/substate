...
//
//	mockgen -source=substate_db.go -destination=./substate_db_mock.go -package=db -imports=mysubstate=github.com/0xsoniclabs/db -self_package=github.com/0xsoniclabs/substate/db
//
...
func (m *MockSubstateDB) NewIterator(prefix, start []byte) ldbiterator.Iterator {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "NewIterator", prefix, start)
	ret0, _ := ret[0].(ldbiterator.Iterator)
	return ret0
}
...
func (m *MockSubstateDB) NewSubstateIterator(start, numWorkers int) Iterator[*substate.Substate] {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "NewSubstateIterator", start, numWorkers)
	ret0, _ := ret[0].(Iterator[*substate.Substate])
	return ret0
}
...
func (m *MockSubstateDB) SetSubstateEncoding(encoding string) (*CSubstateDB, error) {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "SetSubstateEncoding", encoding)
	ret0, _ := ret[0].(*CSubstateDB)
	ret1, _ := ret[1].(error)
	return ret0, ret1
}
